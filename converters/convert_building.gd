...
func _init() -> void:
	...
	if output.is_empty():
		output = "res://assets/converted/buildings".path_join(building).path_join("%s.scn" % building)

	var builder = BuildingBakeBuilderScript.new()
	builder.building_model_dir = String(args.get("source", "res://assets/raw_original_content/3DDATA/Buildings"))
...
func _usage(error: String) -> void:
	...
	print("Usage:")
	print("  godot --headless --path . --script res://converters/convert_building.gd -- --building ATBarracks")
	print("Options:")
