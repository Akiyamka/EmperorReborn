...
func nearby(position: Vector3, buckets: Dictionary, search_radius := UnitNavigationSystem.CELL_BUCKET_SIZE) -> Array:
	var center := bucket_key(position)
	var result := []
	var bucket_radius := maxi(1, ceili(search_radius / UnitNavigationSystem.CELL_BUCKET_SIZE))
...
func bucket_key(position: Vector3) -> Vector2i:
	return Vector2i(
		floori(position.x / UnitNavigationSystem.CELL_BUCKET_SIZE),
		floori(position.z / UnitNavigationSystem.CELL_BUCKET_SIZE)
	)
