...
### [ ] Этап 2. Безопасное перемещение по feature-папкам (2A завершен, остальные batches pending)
...
#### [ ] Этап 2B. Players/rules feature files (pending)

Scope: отдельным механическим batch определить и переместить player/technology/rules paths вместе с `.uid`; API и поведение не менять.

#### [ ] Этап 2C. Units/buildings/UI feature files (pending)

Scope: только после отдельной инвентаризации сцен, raw/generated references и границ этапов 3-4; не смешивать move с разделением controller/UI.

#### [ ] Этап 2D. Map runtime files (pending)

Scope: согласовать с разделением runtime/converters этапа 5; generated map assets после path changes только регенерировать штатным converter.
