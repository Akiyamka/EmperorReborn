...
Последним отдельным batch после стабилизации queue/placement переместить `scripts/building.gd(.uid)` в `scripts/buildings/building.gd(.uid)` с сохранением UID, обновить `converters/building_bake_builder.gd` и штатно регенерировать используемые generated binary building scenes; generated output вручную не редактировать.
