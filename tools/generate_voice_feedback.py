...
def manifest_text(profile_paths: dict[str, str]) -> str:
    lines = [
...
def main() -> int:
    ...
    events = voice_events(parse_sources())
    wavs = wav_lookup()
    ...
    ok = write_or_check(MANIFEST_PATH, manifest_text(profile_paths), args.check) and ok

    if not args.check:
        remove_stale(EVENT_DIR, expected_events)
        remove_stale(PROFILE_DIR, expected_profiles)
        print(f"generated {len(events)} voice events and {len(by_profile)} voice profiles")
    if missing:
        count = sum(len(samples) for samples in missing.values())
        print(f"warning: {count} referenced voice samples are absent from the WAV archive")
