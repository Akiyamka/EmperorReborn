...
### Этап 8 — `match/`, `harvester.gd`, `ui/`, `map_spice_layer.gd` — **сделано**
Точечные разрезы по списку выше. Самостоятельны и могут идти параллельно/позже.

Три дедупа из начала раздела (`autoload_lookup.gd`, `entity_query.gd`, `static var`
в каталогах) закрыты ещё этапом 1 — здесь остались только точечные разрезы.

#### Что сделано

| Файл | Было → стало | Разрез |
|---|---|---|
| `map_spice_layer.gd` | 677 → 448 | три модуля: `map_spice_spread.gd` (188), `map_spice_hazard.gd` (160), `map_spice_render.gd` (126) |
| `unit_command_controller.gd` | 872 → 894* | `selection_partition.gd` (35), `_controllable_entities()`, `_movement_label()`, `_deploy_cursor_for()` |
| `side_panel.gd` | 459 → 457* | четыре пары building/upgrade-близнецов → один путь по `OptionKind` |
| `cursor_manager.gd` | 503 → 493* | один `_build_model_viewport()` вместо двух копий стека; `_ensure_model_pair()` |
| `harvester.gd` | 830 → 835* | `_is_harvest_animating()`/`_is_unload_animating()` вместо 13 копий идиомы |
| `unit_deployment_controller.gd` | — | `_reject(message)` вместо шести словарей одной формы |
| `match.gd` | — | `_local_player()` вместо пятикратной преамбулы |

\* строк стало столько же или больше: копипаста ушла, но каждый разрез объяснён
комментарием «почему так, а не иначе». Дублирования нет — растут доки, не код.

#### Правила, которые здесь подтвердились

- **классовый цикл.** `MapSpiceLayer.hazard()/spread()` **не** аннотированы типом:
  модули объявляют владельца как `MapSpiceLayer`, и обратная аннотация закрывает цикл,
  который парсер не резолвит (`Could not resolve external class member`). Типизировать
  `_layer` базовым `RefCounted` — не выход: `:=` перестаёт выводиться на каждом
  дак-вызове. То же самое, почему юнит-модули держат `CharacterBody3D`, а не `Unit`.
- **сигналы остаются на владельце** (то же, что на этапе 7): модули зовут
  `layer.emit_spread_stage()/emit_spread_finished()`, свои сигналы не заводят.
- **запись пачкой — не оптимизация, а семантика.** `add_spice_batch()` существует
  потому, что кольцо спреда обязано быть одной загрузкой маски и одной серией сигналов
  *после* того, как сетка дописана: слушатель не должен видеть половину кольца.

#### Чего сделано не было — и почему

- **`harvester_phase_machine.gd` (две машины в одну).** Машины параллельны по форме, но не
  по поведению: unload-драйвер начисляет кредиты посегментно, ограничен 256 переходами и
  выходит, как только исчерпана дельта кадра; harvest-драйвер делает до 4 переходов и
  намеренно продолжает съедать нулевые фазы после исчерпания дельты (иначе отсутствующий
  клип вешает машину). Слияние требует колбэка и флага режима — машинерии больше, чем
  сэкономленных ~30 строк, и начисление кредитов в ней теряется. Сделаны только настоящие
  дубли: идиома «идёт анимация» (13 копий) и два буквальных алиаса
  `_start_action_animation`.
- **шесть фабрик `_setup_*` в `match.gd`.** Общего у них ровно три строки
  (`new()` / `name` / `add_child`), а вынос в хелпер, возвращающий `Node`, лишает
  статического типа четыре типизированных поля контроллеров.
- **унификация формы результата.** `{handled, started, message}` (стратегии деплоя) и
  `{handled, available, message}` (проба кандидата) — разные контракты («приказ начат» vs
  «опция предлагаема»), а не три спеллинга одного. Сведены только шесть копий внутри
  `_deployment_candidate`.

#### Про тесты

- `side_panel.gd`: upgrade-ветка была покрыта частично, поэтому слиянию предшествовал
  отдельный коммит с шестью ассертами на неё (и на building-половину той единственной
  асимметрии, ради которой пары вообще различались: `DISABLED` у здания сохраняет ячейку,
  у апгрейда — прячет). Все шесть зелёные на коде *до* слияния.
- `tests/maps/run.gd` переведён с пяти приватных методов, вызывавшихся по имени, на API
  модулей (`layer.spread().create_job(...)`, `layer.hazard().damage_per_second()`) — по
  решению этапа 7, forwarding-обёртки не заводились.
- `tests/units/harvester_run.gd` переопределял `_start_harvest_animation` и
  `_start_unload_animation`, чтобы дать двум машинам разную длину фейкового клипа. Это
  различие перенесено на имя клипа — единственное место, где алиасы были не мертвы.
...
## Отдельная задача: `Harvester` — с наследования на композицию — **сделано**
...
### Готовность — выполнена

`harvester_run.gd` (103 ассерта), `demo_boot_run.gd`, `navigation/run.gd` зелёные;
`grep -rn "extends Unit" scripts/` пуст. `make godot-perf` p50 7.25 мс против 7.12 на той
же машине несколькими минутами раньше — шум, регрессии нет.

#### Как решился вопрос «а кто такой харвестер»

Не типом сцены и не подклассом, а данными: **ёмкость под спайс и есть ответ**. Сеттер
`Unit.max_spice` прицепляет модуль, обнуление — отцепляет.

Именно это поле и никакое другое. «Ёмкость» в проекте не одна: у APC будет вместимость
пехоты (`max_passengers` — `selection_halo.gd:134` уже читает его отдельно от `max_spice`),
у орнитоптера — боезапас. Ни то, ни другое не имеет отношения к «доехать до поля и есть
его», и через этот сеттер их проводить нельзя. Если понадобится трюм без добычи или добыча
без трюма — менять надо `_sync_harvester_controller()`, а не смысл `max_spice`.

Отсюда три следствия:

- `harvester.tscn` сохраняет свою модель и `config_id`, но несёт `unit.gd`, как любая
  другая сцена юнита;
- тестовая фикстура, которая раньше обязана была быть подклассом, теперь получает рабочий
  харвестер, просто выставив ёмкость;
- `ORIGINAL_SPICE_CAPACITY` (fallback 700 для определения без колонки `SpiceCapacity`)
  удалён: колонка есть (`Harvester.tres` = 700), а «нет ёмкости **под спайс**» теперь
  значит «не харвестер», так что подставлять нечего.

Груз (`spice`/`max_spice`) остался на фасаде рядом с health/shields — по той же причине,
по которой там остались они: `match_snapshot` читает `spice` по имени, а `selection_halo`
рисует по `max_spice` своё кольцо.

#### Что пришлось добавить сверх плана

Отдельным коммитом **до** переезда — узкое навигационное API `Unit`
(`is_navigation_managed`, `navigation_command_dock/depart/move`,
`navigation_arrival_tolerance`) вместо четырнадцати мест, где наследник лез в
`_navigation_system`/`_navigation_managed` и каждый раз повторял гвард «managed, не null,
и есть ли такой метод». Плюс `Unit.play_action_animation()` — это была unit-level
способность, жившая в наследнике. Порядок (сначала API против ещё живого наследника,
потом переезд) выбран, чтобы обе половины были зелёными по отдельности.
