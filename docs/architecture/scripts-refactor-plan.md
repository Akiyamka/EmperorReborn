...
Не типом сцены и не подклассом, а данными: **ёмкость трюма и есть ответ**. Сеттер
`Unit.max_spice` прицепляет модуль, обнуление — отцепляет. Отсюда три следствия:
...
- `ORIGINAL_SPICE_CAPACITY` (fallback 700 для определения без колонки ёмкости) удалён:
  колонка есть (`Harvester.tres` = 700), а «нет ёмкости» теперь значит «не харвестер»,
  так что подставлять нечего.
